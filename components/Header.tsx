    ...
    return (
        <nav
            className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ease-in-out border-b ${isScrolled
                ? 'py-4 bg-black/80 backdrop-blur-xl border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.1)]'
        ...
        >
            <div className="max-w-[1600px] mx-auto px-6 md:px-12 flex items-center justify-between relative">
                ...
                <div className={`flex items-center gap-6 md:gap-8 transition-colors duration-500 ${isScrolled ? 'text-white' : 'text-black'}`}>
                    ...
                    <button
                        onClick={toggleCart}
                        className="relative hover:text-[#ff6a3d] transition-colors group"
                    >
            ...
            >
                {NAV_LINKS.map((item, i) => (
                    <Link
                        ...
                        className={`text-3xl font-rajdhani font-semibold tracking-[0.2em] text-white transition-opacity duration-300 hover:text-[#ff6a3d] ${isMobileMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
